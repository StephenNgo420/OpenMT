...
- **A small Hetzner VPS** (decided on over a laptop or Raspberry Pi — see
  ...
  24/7, so the Telegram bots stay online. Nobody can provision or SSH into
...
`~/.openclaw/agents/<id>/workspace/` on the server during setup.
...
3. **Provision the Hetzner server and install OpenClaw on it** — `docs/01-server-setup.md`.
